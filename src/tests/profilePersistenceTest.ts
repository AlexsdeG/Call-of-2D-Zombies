...
export async function runProfilePersistenceTest() {
    ...
    useGameStore.getState().setProfile(initialProfile);
    
    // 2. Simulate Game Start 1
    console.log('STEP 1: Starting Game 1');
    useGameStore.getState().resetSessionStats(); // Verify this resets properly
    let stats = useGameStore.getState().playerStats;
    ...
    useGameStore.getState().updatePlayerStats({ kills: 1 });
    ...
    stats = useGameStore.getState().playerStats;
